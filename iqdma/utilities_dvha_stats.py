...
def widen_data(
    ...
    if date_col is None:
        ...
        keys = ["uid", "date"] + x_variables
        if date_col_backup in keys:
            keys.pop(keys.index(date_col_backup))
        for row in range(len(wide_data["date"])):
            for key in keys:
                final_data[key].append(wide_data[key][sorted_indices[row]])
